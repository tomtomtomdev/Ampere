...
5. **Settings** — keyword, price min/max, scoring weights (W_PERF/W_BATT + per-metric),
   condition filter, **Mall-only toggle**, min seller trust, source selection, **schedule status
   (last run, next run)**, and a manual **Run now** as a fallback only (the daily fetch is
   automatic — see §8a).

## 8a. Scheduling — automatic daily fetch

The daily fetch runs **automatically once per day; the UI button is only a fallback**, not the
normal path. Because runs are idempotent per `snapshot_date` (SC6), triggering twice in a day is
harmless.

- **Trigger:** a headless entrypoint (`run_daily`), invoked by an OS scheduler — **launchd
  LaunchAgent** on macOS (`StartCalendarInterval`, e.g. 06:00 daily) as primary; **cron** as the
  Linux equivalent. Both call the same entrypoint that the Run-now button calls, so there is one
  code path. Prefer the OS scheduler over an in-process timer: it survives app restarts and does
  not require the web server to be running.
- **Catch-up on launch:** a laptop asleep at the scheduled time would otherwise miss the day. On
  app/server startup, if there is no successful run for today's `snapshot_date`, run once
  immediately. This makes "once per day" hold even on an intermittently-on machine.
- **Source note:** the Shopee `InternalEndpointSource` needs a logged-in browser session, so the
  scheduled job launches Playwright with the persisted profile (Appendix A). If the session is
  stale/expired, the run fails cleanly (transactional, SC6), logs it, and the next trigger or
  catch-up retries — no partial snapshot.
- **Observability:** every run writes a `runs` row (started/finished/status/source_kind/count);
  Settings shows last-run + next-run; a failed run surfaces a non-blocking banner.
...
- SC8. The daily fetch runs **automatically once per day without manual action** (OS scheduler),
  and a machine that was asleep at the scheduled time still gets exactly one run for that date via
  launch-time catch-up — never zero, never a duplicate (idempotent per `snapshot_date`).
